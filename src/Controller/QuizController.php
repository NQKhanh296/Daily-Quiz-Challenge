...
class QuizController extends AbstractController
{
    ...
    public function start(Request $request): JsonResponse
    {
        ...
        $attempt->setLastInteractionAt(new \DateTimeImmutable()); 

        $this->entityManager->persist($attempt);
        $this->entityManager->flush();

        $firstQuestion = $quiz->getQuestions()[0];
    ...
    public function submitAnswer(Request $request): JsonResponse
    {
        ...
        $quiz = $this->quizRepository->findOneBy(['date' => new \DateTimeImmutable('today')]);
        $attempt = $this->attemptRepository->findOneBy(['user' => $user, 'quiz' => $quiz, 'isCompleted' => false]);
        ...
        $duration = $now->getTimestamp() - $attempt->getLastInteractionAt()->getTimestamp();
